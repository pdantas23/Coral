...
router.get("/", async (req, res) => {
  try {
    ...
    const promocao =
      req.query.promocao === "true"
        ? true
        : req.query.promocao === "false"
        ? false
        : undefined;
...
// ─── POST /api/products ───────────────────────────────────────────────────────
router.post("/", requireAuth(["comercial"]), async (req, res) => {
  try {
...
router.delete("/delete-image", requireAuth(["comercial"]), async (req, res) => {
  try {
    const { path } = req.body;
...
router.delete("/:id", requireAuth(["comercial"]), async (req, res) => {
  try {
